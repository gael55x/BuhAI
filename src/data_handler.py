...
class DataHandler:
    ...
    def get_prediction_features(self, end_time: datetime, window_minutes: int = 90) -> pd.DataFrame:
        ...
        activity_query = self.session.query(ActivityLog).filter(
             ActivityLog.timestamp_start.between(start_time - timedelta(minutes=60), end_time) 
        ).statement
        ...
        if not df_sleep.empty:
            ...
            df_cgm['sleep_quality'] = quality_map.get(last_sleep['sleep_quality'], 0.5) 
        else:
            df_cgm['sleep_quality'] = 0.5 
    ...
    def get_historical_meal_estimate(self, food_description: str) -> float:
        ...
        final_estimate = avg_peak + (quantity - 1) * (avg_peak * 0.25) 
